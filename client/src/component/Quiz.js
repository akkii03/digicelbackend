...
export default function Quiz() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("token")) {
      axios.get("https://digicelserver.herokuapp.com/user/ques").then((res) => {
        setQues(res.data);
        setLoading(false);
      });
    } else {
          ...
          <>
            ...
            {index >= 10 ? (
              ...
              <div class="row align-items-center">
                ...
                <div className="row-8">
                  <button className="btn btn-danger" onClick={prevQues}>
                    prev
                  </button>

                  <button className="btn btn-success" onClick={nextQues}>
                    next
                  </button>
                </div>
              </div>
